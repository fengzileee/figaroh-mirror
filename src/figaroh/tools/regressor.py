...
def build_total_regressor_current(W_b_u, W_b_l,W_l, I_u, I_l,param_standard_l, param):
    ...
    for ii in range(1,nb_j):
        V_a_ii=np.concatenate((np.concatenate((np.zeros((param['nb_samples']*(ii),1)),I_u[param['nb_samples']*(ii):(ii+1)*param['nb_samples']].reshape(param['nb_samples'],1)), axis=0),np.zeros((param['nb_samples']*(nb_j-1-(ii)),1))), axis=0)
        V_b_ii=np.concatenate((np.concatenate((np.zeros((param['nb_samples']*(ii),1)),I_l[param['nb_samples']*(ii):(ii+1)*param['nb_samples']].reshape(param['nb_samples'],1)), axis=0),np.zeros((param['nb_samples']*(nb_j-1-(ii)),1))), axis=0)
    ...
    # [Ixx, Ixy, Ixz, Iyy, Iyz, Izz, mx, my, mz, m, ia, fv, fs, off]
    if param['has_friction']:
        ...
        W_kpayload_m = np.concatenate((np.zeros((len(W_l),1)),-param['mass_load']*W_l[:,(param['which_body_loaded'])*14+9].reshape(len(W_l),1)), axis=0)
        W_tot = np.concatenate((W_tot,W_kpayload_m), axis=1) 
        print(W_tot.shape,'rank',np.linalg.matrix_rank(W_tot))
...
# def build_total_regressor_current(W_b_u, W_b_l,W_l, I_u, I_l, param):
   
#     nb_j=6
#     param['nb_samples'] = int(len(I_u)/nb_j)
...
#     W_tot=np.concatenate((W_current,W), axis=1)
    # print(W_tot.shape,'rank',np.linalg.matrix_rank(W_tot))

#     W_e_l=np.zeros((len(W_l),10))
#     for k in [1,2,3,4,5,6,7,8,9]:
#         W_e_l[:, k]=W_l[:,(param['which_body_loaded'])*14 + k]
#     W_upayload = np.concatenate((np.zeros((len(W_l),W_e_l.shape[1])),-W_e_l), axis=0)
#     W_tot = np.concatenate((W_tot,W_upayload), axis=1) 
#     W_kpayload = np.concatenate((np.zeros((len(W_l),1)),-param['mass_load']*0.5*W_l[:,(param['which_body_loaded'])*14].reshape(len(W_l),1)), axis=0)
#     W_tot = np.concatenate((W_tot,W_kpayload), axis=1) 
...
#     if np.abs(X_tot[-1]) > 1e-10:
#         X_tot = X_tot / X_tot[-1]
#     else:
#         print("Warning: Small denominator in normalization")

#     residue=np.matmul(W_tot,X_tot)

#     return W_tot, X_tot, residue
